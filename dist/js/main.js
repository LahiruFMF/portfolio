...
(function ($) {
    ...
    $('.gallery')
        .on('click', '.gitem', function (event) {
            ...
            portTitle = $(this).find('.port-title').html();
            portDescription = $(this).find('.port-description').html();
            portRef = $(this).find('.reference').html();
            portPrimaryColors = $(this).find('.primary-color-list').html().split('#');
            portDesignTechList = $(this).find('.design-tech-list').html().split('#');
            portDevTechList = $(this).find('.dev-tech-list').html().split('#');
            onModal = true;
            var $a = $(this),
        ...
        .on('click', '.modal', function (event) {
            ...
            if (event.target.className == 'modal visible loaded') {
                var $modal = $(this),
                    $modalImg = $modal.find('img');
        ...
        .append('<div class="modal" tabIndex="-1"><div class="inner"><div class="image-wrapper"> <img src="" /></div><div class="content-wrapper"></div>')
        .find('img')
        .on('load', function (event) {
            ...
            var devTechContent = '';
            if(portDevTechList.length>1){
                devTechContent='<div class="label-heading">For Front-end development</div>';
            for (var i = 1; i < portDevTechList.length; i++) {
                devTechContent += '<span class="tech-logo ' + portDevTechList[i] + '" title="' + techArry[portDevTechList[i]] + '"></span>';
            }
            }

            var designTechContent = "";
            if(portDesignTechList.length>1) {

                designTechContent = '<div class="label-heading">For design</div>';
                for (var i = 1; i < portDesignTechList.length; i++) {
                    designTechContent += '<span class="tech-logo ' + portDesignTechList[i] + '" title="' + techArry[portDesignTechList[i]] + '"></span>';
                }
            }

            var colorsContent = "";
            for (var i = 1; i < portPrimaryColors.length; i++) {
                colorsContent += '<span class="primary-color" style="background-color:#' + portPrimaryColors[i] + '"></span>';
            }
            var content = '<div class="main-heading">' + portTitle + '</div>' +
                '<div class="content-description"><p>' + portDescription + '</p></div>' +
                '<div class="sub-content">' +
                '<div class="sub-heading">Primary colours</div>' +
                '<div class="content"><div class="color-line">' +
                colorsContent +
                '</div></div>' +
                '</div>' +
                '<div class="sub-content">' +
                '<div class="sub-heading">Technologies used</div>' +
                '<div class="content">' +
                designTechContent +
                devTechContent +
                '</div>' +
                '</div>' +
                '<div class="sub-content">' +
                '<div class="sub-heading">Reference Link</div>' +
                '<div class="content"><a class="url" href="'+portRef+'" target="_blank" >' + portRef + '</a> </div>' +
                '</div>' +
                '</div></div>';
            $(this).parent().next('.content-wrapper').empty();
            $(this).parent().next('.content-wrapper').append('<div>' + content + '</div>');
            setTimeout(function () {
